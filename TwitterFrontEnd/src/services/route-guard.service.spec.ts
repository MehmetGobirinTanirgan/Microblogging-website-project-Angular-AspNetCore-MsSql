...
describe('Service: RouteGuard', () => {
  ...
  let router: Router;
  beforeEach(() => {
    const authServiceSpyObj = jasmine.createSpyObj('AuthenticationService', ['isLoggedIn']);
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([])],
      providers: [RouteGuardService, { provide: AuthenticationService, useValue: authServiceSpyObj }],
    });
  ...
  it('#canActivate should return true if user logged in', () => {
    ...
    const _canActivate = service.canActivate(new ActivatedRouteSnapshot(), <RouterStateSnapshot>{ url: 'mockURL' });
